...
#endif
    if (rc == TPM_RC_SUCCESS) {
        BLOCK_SKIP_READ(skip_accumulate_self_heal_timer, needs_block, buffer, size,
                        "Volatile state", "s_selfHealTimer");
    }
#ifndef ACCUMULATE_SELF_HEAL_TIMER
    if (rc == TPM_RC_SUCCESS) {
...
#endif
skip_accumulate_self_heal_timer:
...
PERSISTENT_ALL_Unmarshal(BYTE **buffer, INT32 *size)
{
    TPM_RC rc = TPM_RC_SUCCESS;
    NV_HEADER hdr;
    PERSISTENT_DATA pd = {0};
    ORDERLY_DATA od = {0};
    STATE_RESET_DATA srd = {0};
    STATE_CLEAR_DATA scd = {0};
    BYTE indexOrderlyRam[sizeof(s_indexOrderlyRam)];
